...
### Compartilhamento de ficha, redesenhado (usuário-a-usuário, só leitura)
- Segundo as respostas: **qualquer usuário cadastrado no app** pode ser escolhido (não só quem já tem ficha nesse sistema — o motivo dado foi mostrar uma ficha antiga/inativa pra um jogador novo, ou pra um mestre usar como base de NPC/monstro, coisas que não implicam o destinatário já ter personagem ali); há um **indicador simples** (badge na tela do sistema mostrando que a ficha está compartilhada, sem sistema de notificação separado); e o link público antigo (`shared`/`shareSlug`) foi **removido por completo**, não só desativado.
- **Nova tabela `character_shares`** (`character_id`, `shared_with_user_id`, índice único no par, índice em `shared_with_user_id`) em vez de guardar isso dentro do blob JSONB da ficha — diferente do resto dos dados do personagem, "quem mais pode ver essa ficha" precisa ser consultável a partir da conta de quem recebeu o acesso, não só de quem é dono, então teria que varrer o JSONB de todo mundo pra responder "quais fichas foram compartilhadas comigo". Mesmo raciocínio já usado pra biblioteca compartilhada (tabela normalizada por causa da relação um-para-muitos entre sistema e itens).
- **Rotas novas** (dentro dos dispatchers existentes, sem passar do limite de funções do plano Hobby): `POST/DELETE /api/characters/:id/shares` (só o dono pode chamar; o destinatário nunca recebe nenhum endpoint de escrita pra essa ficha) e `GET /api/user/roster` (lista leve de todo mundo, pro seletor de usuário no popup de compartilhar).
- `GET /api/bootstrap` passou a devolver também `sharedWithMe` (fichas que compartilharam comigo, com nome/usuário de quem é dono) e `mySharesByCharacterId` (com quem cada ficha minha está compartilhada, pra eu poder gerenciar).
- **Garantia de "nunca editável"**: o hook `useCharacter` (que toda a ficha usa pra ler/editar) agora também resolve personagens de `sharedWithMe`, sempre com `readOnly: true` — e pra esses, a função `update` que ele devolve é um **no-op de verdade** (só um aviso no console), não lança erro nem localmente muda nada. Como TODOS os popups de edição da ficha recebem esse mesmo `update` (repassado pela aba), essa é a garantia de fundo: mesmo que um popup específico esqueça de esconder o botão "Editar", nenhuma edição consegue de fato acontecer nem localmente nem no servidor.
- **Visual**: o cabeçalho da ficha (`SheetHeader`) já esconde nome editável, troca de foto, favoritar/ativar e mostra uma faixa "Somente leitura — ficha de @usuário" quando a ficha é compartilhada. **Pendência conhecida**: os 16 popups de edição de campo (identidade, atributos, perícias, etc.) ainda só escondem o botão "Editar" com base em `character.active`, não em `readOnly` — então tecnicamente ainda aparecem clicáveis numa ficha compartilhada, mesmo que clicar e editar não tenha efeito nenhum (o `update` no-op garante isso). É só uma lacuna visual, não de segurança/integridade de dados — fica registrado pra um próximo passo se for importante deixar 100% visualmente consistente.
- Nova seção **"Fichas compartilhadas"** na tela de cada sistema (`CharactersPage`), abaixo dos personagens próprios, mostrando de quem é cada ficha (`de @usuário`).

### Snapshots por nível, de verdade (árvore com forks, soft delete, restauração)
- Segundo as respostas: snapshot automático a cada subida de nível; guarda o estado **completo** da ficha (não só um resumo); permite **restaurar**; ao restaurar, cria antes um snapshot do estado atual (pra não perder nada); o que ficava "acima" do ponto restaurado continua existindo, mas some da linha do tempo principal e vai pra uma aba separada; soft delete (nada é apagado de verdade, só marcado); snapshot que faz parte da linha do tempo atual **não pode** ser excluído; a snapshot da restauração (o "antes de restaurar") **pode** ser excluída; upar de nível depois de restaurar gera duas snapshots (a da restauração + a do novo nível), naturalmente, sem precisar de um caso especial; quis uma visualização em árvore num popup de detalhes.
- **Modelo de dados**: `LevelSnapshot` ganhou `parentId` (encadeia snapshots numa árvore, não numa lista — restaurar um snapshot antigo bifurca a linha do tempo), `kind` (`'level-up' | 'restore-point'`), `deletedAt` (soft delete) e `data` (a ficha inteira naquele momento, exceto os campos de snapshot dela mesma, pra não aninhar a árvore dentro dela mesma recursivamente). Novo campo `Character.currentSnapshotId` marca de qual snapshot a ficha viva "descende" agora — é assim que dá pra saber qual é a "linha do tempo ativa" (anda pra trás pelos `parentId` a partir dele) sem precisar de um campo separado tipo "branch atual".
- **Regra de proteção contra exclusão, unificada**: um snapshot só pode ser excluído se **não for ancestral** do `currentSnapshotId` — isso sozinho já cobre as duas regras que pareciam precisar de casos especiais: a linha do tempo ativa fica sempre protegida (não dá pra excluir onde você realmente esteve), e o "antes de restaurar" nasce automaticamente fora da linha ativa (ele é folha de um ramo abandonado), então já nasce excluível, sem precisar marcar isso à parte.
- **Restaurar**: tira um snapshot do estado atual (`kind: 'restore-point'`), sobrescreve os dados da ficha com os do snapshot escolhido, e move o `currentSnapshotId` pra ele. Nada é perdido — o ramo antigo continua inteiro na árvore, só deixa de ser "o presente".
- Aba Histórico dividida em "Linha do tempo atual" (protegida) e "Outros ramos" (excluível), e um popup de detalhes por snapshot com estatísticas resumidas, botão de restaurar/excluir, e a árvore completa (todos os ramos, indentada por profundidade, com o nó atual e o que está sendo visto destacados).
- Escrevi 7 testes automatizados (`entities/character/model/snapshots.test.ts`) cobrindo especificamente os cenários mais delicados do fork: restaurar preserva o ramo antigo com os níveis certos, upar de nível depois de restaurar continua o ramo novo (não o antigo), e a proteção contra exclusão muda corretamente antes/depois de uma restauração — como não dá pra testar isso interagindo com o navegador neste ambiente, esses testes foram a forma de validar a lógica de verdade antes de entregar.

### Correções do review automático do GitHub Copilot na PR das mudanças acima
- O Copilot revisou a PR e apontou 6 problemas formais (3 "High", 3 "Medium"), todos reais depois de conferidos contra o código — corrigidos nesta passada:
  1. e 2. **Ficha de antes do recurso de snapshots quebrava a aba Histórico**: `levelSnapshots`/`currentSnapshotId` nasceram como campos obrigatórios no tipo `Character`, mas qualquer personagem salvo no banco antes dessa mudança simplesmente não tem esses campos no JSONB — não existe nenhuma migração de dados retroativa. `activeLineage`/`otherBranches`/etc. faziam `character.levelSnapshots.map(...)` direto, o que quebraria com `undefined` pra qualquer ficha antiga. Corrigido em duas camadas: `normalizeCharacter` (novo, em `entities/character/model/factory.ts`) preenche os defaults (`[]`/`null`) assim que os personagens chegam da API em `AppDataProvider` (tanto os próprios quanto os `sharedWithMe`), e as funções de `snapshots.ts` também passaram a se defender sozinhas (`?? []`, `?? null`, e um filtro `wellFormedSnapshots` que ignora qualquer linha sem `data`) — dupla proteção, já que uma ficha nunca deveria depender de passar por um único ponto de entrada pra não quebrar. `SnapshotDetailDialog` e `SnapshotTree` também passaram a ignorar/guardar contra uma entrada sem `data`.
  3. **`discardChanges` sobrescrevia a ficha inteira — e o cenário é real, não hipotético**: `update(() => snapshot)` ignorava completamente o estado atual da ficha e substituía tudo pelo snapshot capturado na abertura do popup. O Copilot apontou um cenário concreto e verificado: `QuickStats` mantém `HpDialog` e `AbilityDialog` como dois popups **independentes**, cada um podendo estar aberto ao mesmo tempo (estados React separados, não uma única aba); editar PV e depois descartar uma edição de atributo (ou vice-versa) apagaria a mudança de PV junto. Minha primeira tentativa (`revertChangedKeys`, comparando `current` com o snapshot campo a campo) não resolvia isso de verdade — se os dois popups mudassem o mesmo campo-raiz por coincidência, ou mesmo com campos diferentes onde um dependia do outro, ainda dava pra reverter algo que não era desse popup, já que "o que difere do snapshot" não distingue "editei isso" de "outro popup editou isso enquanto eu estava aberto". Corrigido de verdade com rastreamento de origem: `useEditableSection` agora também devolve seu próprio `update` (`trackedUpdate`), que envolve o `update` de verdade e registra exatamente quais chaves de nível raiz do `Character` cada chamada muda; esse é o `update` que os 16 popups agora usam pra qualquer edição de campo (troquei todos os 16 arquivos). `discardChanges` passou a chamar `revertKeys(current, snapshot, touchedKeys)` — reverte só as chaves que ESTE popup de fato tocou, nunca uma chave que outro popup mudou enquanto os dois estavam abertos.
  4. **Migração `0010` não era idempotente**: rodar de novo travaria em "already exists". Reescrita com `CREATE TABLE IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`, e as duas `ALTER TABLE ... ADD CONSTRAINT` envolvidas em blocos `DO $$ ... EXCEPTION WHEN duplicate_object THEN NULL; END $$;` — mesmo padrão já usado nas outras migrações idempotentes deste projeto.
  5. **Excluir um snapshot no meio da árvore "órfãava" os filhos dele na visualização**: `SnapshotTree` agrupava filhos por `parentId` bruto — se o pai fosse soft-deletado, os filhos dele (que não foram excluídos) sumiam da árvore inteira, mesmo continuando presentes nos dados. Corrigido andando `parentId` acima até achar o ancestral mais próximo que ainda está visível (não deletado, com `data`), reconectando os filhos a ele (ou virando raiz, se não sobrar nenhum ancestral visível).
  6. **Ficha inativa entrava em modo de edição sozinha**: o gatilho "abre direto editando quando a seção está vazia" (`isEmpty`) não conferia `character.active` — então um popup de uma ficha arquivada podia entrar em edição mesmo a ficha sendo, por design, só-leitura enquanto inativa. `useEditableSection` agora exige `isEmpty && character.active`, e `setEditing(true)` virou no-op quando a ficha está inativa.
- Aproveitei a mesma passada pra corrigir observações informais do Copilot (fora das 6 formais, mas com a mesma causa raiz — falta de guarda por `character.active`/`readOnly`, ou tratamento de erro incompleto): botão de trocar foto no `SheetHeader` passou a exigir `canEdit` (antes só checava `!readOnly`, deixando trocar foto de ficha inativa); botão "Adicionar grimório" e os controles do `SpellBookCard` (buscar magia, editar espaços, remover) agora respeitam `character.active`; `SharePickerDialog` não fica mais preso em "Carregando…" pra sempre se `GET /user/roster` falhar (mostra uma mensagem de erro e permite tentar de novo reabrindo o popup); `setCircleMax` não deixa mais reduzir o máximo de espaços de magia abaixo de quantos já foram usados nesse círculo; e `shareCharacter`/`unshareCharacter` no `AppDataProvider` passaram a rastrear um número de sequência por ficha, então se as respostas de duas chamadas chegarem fora de ordem, a mais antiga não sobrescreve o estado deixado pela mais nova.
- Testes novos: `entities/character/model/factory.test.ts` (normalização de ficha legada), dois testes a mais em `snapshots.test.ts` (ficha sem os campos de snapshot, e uma linha de snapshot sem `data`), e `useEditableSection.test.ts` (comportamento de `revertKeys` isolado da parte de React/hooks, que não dá pra testar sem montar componente neste ambiente sem navegador de verdade).

### Cor do "Modificador" no popup de atributo não deveria seguir a cor da habilidade
...
## Dados do personagem: de um blob JSONB para tabelas relacionais

O usuário rejeitou explicitamente a modelagem original de `characters.data` como uma única coluna `jsonb` guardando todo o `Character` ("NADA DE JSONB! essa é uma péssima modelagem de dados!") e pediu uma modelagem estruturada, com mais de uma tabela. Perguntado sobre o escopo, o usuário escolheu: normalização total (cada seção de lista vira sua própria tabela), sem necessidade de preservar dados existentes em produção (pode resetar), e snapshots de nível guardados como blob serializado só para histórico.

- `db/schema.ts`: a coluna `characters.data` (jsonb) foi removida. Os campos 1:1 de cardinalidade fixa (`identity`, `speed`, `saves`, `money`, `load`) viraram colunas próprias na tabela `characters` — não há razão relacional para uma tabela separada quando não há repetição. Toda seção que é uma lista no tipo `Character` virou sua própria tabela com FK `character_id → characters.id ON DELETE CASCADE`: `character_classes`, `character_languages`, `character_hp_log`, `character_dr_items`, `character_abilities` (+ `character_ability_mods`, `character_ability_log`), `character_ac_varied_mods`, `character_init_varied_mods`, `character_spellbooks` (+ `character_spell_circles`), `character_skills` (+ `character_skill_mods`), `character_conditional_mods`, `character_weapons` (+ `character_weapon_ammo_log`), `character_feats`, `character_special_abilities` (cobre tanto `spellLikeAbilities` quanto `specials`, diferenciados por uma coluna `kind`), `character_equipment`, `character_armor_items` e `character_session_log`. Cada tabela filha tem uma coluna `sort_order` porque o Postgres não garante ordem de linhas e a ordem dos arrays importa na UI (drag-and-drop).
- **Exceção deliberada**: `character_snapshots` guarda o campo `data` como texto serializado (`JSON.stringify`), não como colunas. Foi a opção que o usuário escolheu explicitamente para o recurso de snapshots — um snapshot é imutável, ponto-no-tempo, e nunca é consultado campo a campo, então normalizá-lo em ~20 tabelas de snapshot a mais seria overhead de modelagem sem ganho relacional real, ao contrário dos dados ao vivo do personagem.
- **Contrato da API não mudou**: `GET /api/bootstrap` e `PATCH /api/characters/:id` continuam recebendo/devolvendo o mesmo objeto `Character` aninhado de sempre — o front-end inteiro (todos os ~35 popups, `AppDataProvider`, cálculos) não precisou mudar uma linha. `api/_lib/characterRepo.ts` é a camada nova que monta esse objeto a partir das tabelas relacionais em batch (uma query por tabela, não uma por personagem) e decompõe o objeto de volta em linhas ao salvar — sempre como substituição completa (delete + insert de tudo), o mesmo contrato de "manda o personagem inteiro a cada mudança" que o front-end já usava contra a coluna jsonb.
- Toda escrita usa `db.batch(...)` (o driver `neon-http` não suporta `db.transaction()` — lança erro "No transactions support in neon-http driver" — mas suporta `batch`, que envia todas as queries como uma única chamada HTTP atômica). Cada linha usa o `id` já gerado pelo cliente (armas, talentos, perícias, etc. já têm `id` no tipo `Character`) como chave primária, então nenhuma query do batch depende do resultado de outra — exceto `character_spell_circles`/`character_skill_mods`/`character_weapon_ammo_log`, cujo pai é inserido antes na mesma lista, e o `parent_id` auto-referenciado de `character_snapshots`, resolvido num segundo passo (`UPDATE` após o `INSERT` de todos os snapshots com `parent_id` nulo) para nunca depender da ordem do array.
- Migração gerada em duas etapas (`db/migrations/0011_drop_characters_data_jsonb.sql` + `0012_add_relational_character_tables.sql`) porque uma única geração misturava um drop com dezenas de adds e o `drizzle-kit generate` tentava resolver isso como possível rename via prompt interativo (que trava num shell não-interativo) — a mesma situação documentada em CLAUDE.md sobre `drizzle-kit generate`.
- Como o usuário autorizou resetar os dados de produção, a migração **não preserva** o conteúdo que já estava em `characters.data`: a coluna é simplesmente descartada. Isso precisa ser aplicado em produção rodando `npm run db:migrate` (não colar SQL manualmente no editor do Neon) para que o Drizzle rastreie a migração como aplicada.

## Bug: nome de usuário duplicado parecia "salvar" na tela de perfil

Reportado pelo usuário: em "Editar dados" no perfil, dava pra colocar um nome de usuário que já pertencia a outra conta. A causa era o padrão geral do app ("toda edição é otimista + chamada à API sem esperar resposta") aplicado a um caso que tem uma rejeição real e esperada do servidor (409, nome de usuário já em uso) — a tela mudava a UI na hora, e o erro do backend só ia pro console, sem reverter nem avisar.
