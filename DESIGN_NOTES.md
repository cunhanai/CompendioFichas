...
- **`vercel.json` `git.deploymentEnabled` — lacuna não verificada**: a intenção é restringir deploy só à `main`, mas não consegui confirmar contra a documentação oficial da Vercel se um branch não listado nesse objeto conta como desabilitado por padrão, ou se precisa de uma regra coringa explícita — tentativa de checar a doc deu bloqueio de rede neste ambiente. Fica registrado como não verificado; a garantia real por enquanto é conferir/ajustar direto no painel (Settings → Git → Preview Deployments).
